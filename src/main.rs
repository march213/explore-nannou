...
impl Model {
    fn new(noise: Option<BasicMulti>, egui: Egui, image_window: WindowId) -> Self {
    ...
    fn update_size(&mut self, size: Vec2) {
        let image = DynamicImage::new_rgb8(size.x.floor() as u32, size.y.floor() as u32);
        self.image = image;
        self.size = size;
    ...
    fn redraw_image(&mut self) {
        for (x, y) in
            (0..self.size.x.floor() as u32).cartesian_product(0..self.size.y.floor() as u32)
        {
            let value = self
                .noise
                .get([x as f64 / NOISE_STEP, y as f64 / NOISE_STEP]);
            if self.settings.grayscale {
                let mapped_value = map_range(value, -1.0, 1.0, 0., 255.);
                ...
                let mapped_hue: f64 = map_range(
                    ...
                    -1.0,
                    1.0,
                    self.settings.hue_center - self.settings.hue_range,
                    self.settings.hue_center + self.settings.hue_range,
                );
                // 0-100, 0-128,181, -180-180
                let color = Lch::new(self.settings.lightness, self.settings.chroma, mapped_hue);
                let rgb_color: Rgb<f64> = nannou::color::rgb::Rgb::from(color);
                let (r, g, b) = rgb_color.into_components();
                let mapped_r = map_range(r, -1.0, 1.0, 0., 255.);
                let mapped_g = map_range(g, -1.0, 1.0, 0., 255.);
                let mapped_b = map_range(b, -1.0, 1.0, 0., 255.);
                if let Some(buffer) = self.image.as_mut_rgb8() {
                    buffer.put_pixel(
                        x,
                        y,
                        nannou::image::Rgb::from([mapped_r as u8, mapped_g as u8, mapped_b as u8]),
                    );
                }
            }
        }
...
fn update(app: &App, model: &mut Model, update: Update) {
    ...
    egui::Window::new("Settings").show(&ctx, |ui| {
        ...
        // Lightness slider
        let slider = egui::Slider::new(&mut settings.lightness, 0.0..=100.0);
        ui.label("Lightness:");
        ...
        ui.label("Chroma:");
        changed |= ui
            .add(egui::Slider::new(&mut settings.chroma, 0.0..=181.0))
            .changed();

        // Chroma slider
        ui.label("Hue Center:");
        changed |= ui
            .add(egui::Slider::new(&mut settings.hue_center, -180.0..=180.0))
            .changed();

        ui.label("Hue range:");
        changed |= ui
            .add(egui::Slider::new(&mut settings.hue_range, 0.0..=180.0))
            .changed();

        changed |= ui
            .add(egui::Checkbox::new(&mut settings.grayscale, "Grayscale"))
            .changed();
        // Random color button
